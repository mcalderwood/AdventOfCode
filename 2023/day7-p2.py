...
def hand_type(hand):
    ...
    if 'J' in card_counts and card_counts['J'] < 5:
        # get card with most duplicates
        #best_card = max(card_counts.items(), key=operator.itemgetter(1))[0]
        best_card = ''
        max_count = 0
        for card in card_counts:
            if card != 'J' and card_counts[card] > max_count:
                max_count = card_counts[card]
                best_card = card

        card_counts[best_card] += card_counts['J']
        del card_counts['J']
